...
namespace BlackCat\Database\Packages\BookCategories\Service;

use BlackCat\Core\Database;
use BlackCat\Database\Packages\BookCategories\Dto\BookCategoryDto;
use BlackCat\Database\Packages\BookCategories\Mapper\BookCategoryDtoMapper;
 ...
 */
final class BookCategoriesAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private BookCategoryRepository $bookCategoryRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'book_categories' => $this->bookCategoryRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\BookCategories\ModuleException("Record {$idStr} not found for locking.");
            }
