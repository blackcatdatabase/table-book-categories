...
use BlackCat\Database\Packages\BookCategories\ContractRepository;

final class BookCategoryRepository implements ContractRepository {
    ...
    public function insert(array $row): void {
        ...
        $sql = "INSERT INTO book_categories (".implode(',', $cols).") VALUES (".implode(',', $place).")";
    ...
    public function insertMany(array $rows): void {
        ...
        $sql = "INSERT INTO book_categories (".implode(',', $cols).") VALUES ".implode(',', $allPlace);
        $this->db->execute($sql, $params);
    }
    ...
    public function upsert(array $row): void {
        ...
        $sqlBase = "INSERT INTO book_categories (".implode(',', $cols).") VALUES (".implode(',', $place).")";
    ...
    public function updateById(int|string $id, array $row): int {
        ...
        $sql = "UPDATE book_categories SET ".implode(',', $assign)." WHERE book_id = :id".$verCond;
    ...
    public function deleteById(int|string $id): int {
        ...
        if ($soft) {
            ...
            return $this->db->execute("UPDATE book_categories SET $set WHERE book_id = :id", $params);
        }
        return $this->db->execute("DELETE FROM book_categories WHERE book_id = :id", [':id'=>$id]);
    }
    ...
    public function restoreById(int|string $id): int {
        ...
        $set = "$soft = NULL";
        if ($updAt && $updAt !== $soft) { $set .= ", $updAt = CURRENT_TIMESTAMP"; }
        return $this->db->execute("UPDATE book_categories SET $set WHERE book_id = :id", [':id'=>$id]);
    }

    // ============ READ / PAGE / LOCK ============
    public function findById(int|string $id): ?array {
        $sql = "SELECT * FROM book_categories WHERE book_id = :id";
        $row = $this->db->fetchOne($sql, [':id'=>$id]);
        return $row ?: null;
    }

    public function exists(string $whereSql = '1=1', array $params = []): bool {
        $sql = "SELECT 1 FROM book_categories WHERE $whereSql LIMIT 1";
        return (bool)$this->db->fetchOne($sql, $params);
    }

    public function count(string $whereSql = '1=1', array $params = []): int {
        $sql = "SELECT COUNT(*) FROM book_categories WHERE $whereSql";
    ...
    public function paginate(Criteria $c): array {
        ...
        $order = $order ?: (Definitions::defaultOrder() ?? 'book_id DESC');
        $joins = $joins ?: '';

        $total = (int)$this->db->fetchOne("SELECT COUNT(*) FROM book_categories t $joins WHERE $where", $params);
        $items = $this->db->fetchAll("SELECT t.* FROM book_categories t $joins WHERE $where ORDER BY $order LIMIT $limit OFFSET $offset", $params);
    ...
    public function lockById(int|string $id): ?array {
        $sql = $this->db->isMysql()
            ? "SELECT * FROM book_categories WHERE book_id = :id FOR UPDATE"
            : "SELECT * FROM book_categories WHERE book_id = :id FOR UPDATE";
