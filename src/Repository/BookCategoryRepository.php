...
use BlackCat\Database\Packages\BookCategories\Repository\BookCategoryRepositoryInterface;
...
class BookCategoryRepository implements BookCategoryRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;

    /** @var mixed literal token for upsert keys (array or empty). */
    private mixed $tokenUpsertKeys = [ 'tenant_id', 'book_id', 'category_id' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\BookCategories\Definitions::class);
    ...
    public function getByTenantIdAndBookIdAndCategoryId(int $tenantId, int $bookId, int $categoryId, bool $asDto = false): array|\BlackCat\Database\Packages\BookCategories\Dto\BookCategoryDto|null {
        return $this->getByUnique([ 'tenant_id' => $tenantId, 'book_id' => $bookId, 'category_id' => $categoryId ], $asDto);
    }
    ...
    public function getIdByTenantIdAndBookIdAndCategoryId(int $tenantId, int $bookId, int $categoryId) {
        $row = $this->getByTenantIdAndBookIdAndCategoryId($tenantId, $bookId, $categoryId, false);
        if (!is_array($row)) { return null; }
        return $row['tenant_id book_id category_id'] ?? null;
    }
