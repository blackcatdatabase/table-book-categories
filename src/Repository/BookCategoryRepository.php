...
namespace BlackCat\Database\Packages\BookCategories\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\BookCategories\Definitions;
use BlackCat\Database\Packages\BookCategories\Criteria;
use BlackCat\Database\Packages\BookCategories\Dto\BookCategoryDto as Dto;
use BlackCat\Database\Packages\BookCategories\Mapper\BookCategoryDtoMapper as RowMapper;
...
final class BookCategoryRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\BookCategories\Definitions::class; }
    ...
    private function doUpsert(array $row, bool $revive): void
    {
        ...
        $keys = [ 'tenant_id', 'book_id', 'category_id' ];
        if (!$keys) {
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [ 'tenant_id', 'book_id', 'category_id' ] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\BookCategories\Definitions::table());
            return $bulk->upsertMany($rows, [ 'tenant_id', 'book_id', 'category_id' ], []);
        }
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\BookCategories\Dto\BookCategoryDto|null */
    public function getByTenantIdAndBookIdAndCategoryId(int $tenantId, int $bookId, int $categoryId, bool $asDto = false): array|\BlackCat\Database\Packages\BookCategories\Dto\BookCategoryDto|null {
        $row = $this->getByUnique([ 'tenant_id' => $tenantId, 'book_id' => $bookId, 'category_id' => $categoryId ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\BookCategories\Mapper\BookCategoryDtoMapper::fromRow($row);
    }
    public function existsByTenantIdAndBookIdAndCategoryId(int $tenantId, int $bookId, int $categoryId): bool {
        $where = 't.' . Ident::q($this->db, 'tenant_id') . ' = :uniq_tenant_id' . ' AND ' . 't.' . Ident::q($this->db, 'book_id') . ' = :uniq_book_id' . ' AND ' . 't.' . Ident::q($this->db, 'category_id') . ' = :uniq_category_id';
        return $this->exists($where, [ 'uniq_tenant_id' => $tenantId, 'uniq_book_id' => $bookId, 'uniq_category_id' => $categoryId ]);
    }
    /** @return int|string|null */
    public function getIdByTenantIdAndBookIdAndCategoryId(int $tenantId, int $bookId, int $categoryId) {
        $row = $this->getByTenantIdAndBookIdAndCategoryId($tenantId, $bookId, $categoryId);
        return $row ? ($row['tenant_id book_id category_id'] ?? null) : null;
    }

    /** @return array<string,mixed>|\BlackCat\Database\Packages\BookCategories\Dto\BookCategoryDto|null */
    public function getByIdForTenant(int|string $id, int|string $tenantId, bool $asDto = false): array|\BlackCat\Database\Packages\BookCategories\Dto\BookCategoryDto|null {
