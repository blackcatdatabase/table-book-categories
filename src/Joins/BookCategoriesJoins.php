...
namespace BlackCat\Database\Packages\BookCategories\Joins;
 ...
 */
final class BookCategoriesJoins {
    ...
    /**
     * FK: book_categories -> categories
     * LEFT JOIN vw_categories AS $as ON $as.tenant_id = $alias.tenant_id AND $as.id = $alias.category_id
     * @return array{0:string,1:array<string,mixed>}
     */
    public function joinCategories(string $alias = 't', string $as = 'j1'): array {
        [$alias, $as] = $this->assertAliasPair($alias, $as);
        return [' LEFT JOIN vw_categories AS ' . $as . ' ON ' . $as . '.tenant_id = ' . $alias . '.tenant_id' . ' AND ' . $as . '.id = ' . $alias . '.category_id' . ' ', []];
    }
